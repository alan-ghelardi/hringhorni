...
import (
	...
	rolloutsv1alpha1 "github.com/nubank/hringhorni/pkg/client/listers/rollouts/v1alpha1"
	zap "go.uber.org/zap"
...
// controller reconciling v1alpha1.Analysis.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.Analysis. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.Analysis) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.Analysis.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.Analysis. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.Analysis) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.Analysis if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.Analysis.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.Analysis) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.Analysis) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1alpha1.Analysis resources.
type reconcilerImpl struct {
	...
	Lister rolloutsv1alpha1.AnalysisLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister rolloutsv1alpha1.AnalysisLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.Analysises(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *v1alpha1.Analysis, desired *v1alpha1.Analysis) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.HringhorniV1alpha1().Analysises(desired.Namespace)
		...
		updater := r.Client.HringhorniV1alpha1().Analysises(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.Analysis, desiredFinalizers sets.String) (*v1alpha1.Analysis, error) {
	// Don't modify the informers copy.
	...
	patcher := r.Client.HringhorniV1alpha1().Analysises(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.Analysis) (*v1alpha1.Analysis, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.Analysis, reconcileEvent reconciler.Event) (*v1alpha1.Analysis, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
